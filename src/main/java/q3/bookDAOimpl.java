...
public class bookDAOimpl implements bookDAO {
	...
	public void createTable() {
		String query = "CREATE TABLE IF NOT EXISTS books (book_id INT PRIMARY KEY, book_title VARCHAR(50), book_price DECIMAL(5,2))";
		try (Connection conn = MyConnection.getConnection()) {
			PreparedStatement prepStmt = conn.prepareStatement(query);
			prepStmt.execute();
			System.out.println("Created table books if it did not exist already");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
